...
namespace core {
    ...
    void Renderer::ExecutePass(Material *material, const GLuint input, const GLuint output) {
    ...
    void Renderer::RenderScene(Scene*& scene, const GLuint& finalFbo, const GLuint& finalTexture, int width, int height) {
        ...
        for (auto& pass : postProcessingEffects) {
            ...
            // set input textures
            GLuint depthTex = ping ? ppTex[0] : ppTex[1];
            pass->SetTexture("_MainTex", input);
            pass->SetTexture("_DepthTex", depthTex);
            pass->Bind();

            quadModel->render();

            input = ping ? ppTex[1] : ppTex[0];
            ping = !ping;
        }
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        glGenRenderbuffers(1, &ppDepth[i]);
